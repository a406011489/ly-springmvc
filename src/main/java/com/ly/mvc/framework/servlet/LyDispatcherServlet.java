package com.ly.mvc.framework.servlet;

import com.ly.mvc.framework.annotations.LyAutowired;
import com.ly.mvc.framework.annotations.LyController;
import com.ly.mvc.framework.annotations.LyRequestMapping;
import com.ly.mvc.framework.annotations.LyService;
import com.ly.mvc.framework.enetity.HandlerMapping;
import com.ly.mvc.framework.util.CommonUtils;

import javax.servlet.ServletConfig;
import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.File;
import java.io.IOException;
import java.io.InputStream;
import java.lang.reflect.Field;
import java.lang.reflect.Method;
import java.lang.reflect.Parameter;
import java.util.*;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

public class LyDispatcherServlet extends HttpServlet {

    /**
     * 需要加载的类的文件夹的路径保存在此配置文件中
     */
    private final Properties properties = new Properties();

    /**
     * 缓存扫描到的类的全限定类名
     */
    private final List<String> classNames = new ArrayList<>();

    /**
     * ioc容器
     */
    private final Map<String, Object> ioc = new HashMap<>();

    /**
     * 存储url和Method之间的映射关系
     */
    private final List<HandlerMapping> handlerMapping = new ArrayList<>();

    @Override
    public void init(ServletConfig config) {

        // 1、加载配置文件 springmvc.properties
        String contextConfigLocation = config.getInitParameter("contextConfigLocation");
        this.doLoadConfig(contextConfigLocation);

        // 2、扫描相关的类，扫描注解
        this.doScan(properties.getProperty("package"));

        // 3、初始化bean对象（实现ioc容器，基于注解）
        this.doInstance();

        // 4、实现依赖注入
        this.doAutoWired();

        // 5、构造一个HandlerMapping处理器映射器，将配置好的url和Method建立映射关系
        this.initHandlerMapping();

        System.out.println("Ly mvc 初始化完成....");
    }

    // 1、加载配置文件
    private void doLoadConfig(String config) {
        InputStream resourceAsStream = this.getClass().getClassLoader().getResourceAsStream(config);
        try {
            properties.load(resourceAsStream);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    // 2、开始扫描调用方的磁盘上的文件夹（File）  com/ly/demo
    private void doScan(String scanPackage) {
        String scanPackagePath = Objects.requireNonNull(Thread.currentThread().getContextClassLoader().getResource("")).getPath() + scanPackage.replaceAll("\\.", "/");
        File filePackage = new File(scanPackagePath);

        File[] files = filePackage.listFiles();

        assert files != null;
        for (File file : files) {

            // 如果是文件夹，则进行递归
            if (file.isDirectory()) {
                this.doScan(scanPackage + "." + file.getName());
            } else if (file.getName().endsWith(".class")) {
                String className = scanPackage + "." + file.getName().replaceAll(".class", "");
                classNames.add(className);
            }
        }
    }

    // 3、基于classNames缓存的类的全限定类名，以及反射技术，完成对象创建和管理
    private void doInstance() {

        if (classNames.size() == 0) return;

        try {
            // com.ly.demo.controller.DemoController
            for (String className : classNames) {
                // 反射
                Class<?> aClass = Class.forName(className);
                // 区分controller，区分service
                if (aClass.isAnnotationPresent(LyController.class)) {

                    // controller的id此处不做过多处理，不取value了，就拿类的首字母小写作为id，保存到ioc中
                    String simpleName = aClass.getSimpleName();// DemoController

                    // demoController
                    String lowerFirstSimpleName = CommonUtils.lowerFirst(simpleName);

                    Object o = aClass.newInstance();
                    ioc.put(lowerFirstSimpleName, o);

                } else if (aClass.isAnnotationPresent(LyService.class)) {
                    LyService annotation = aClass.getAnnotation(LyService.class);

                    //获取注解value值
                    String beanName = annotation.value();

                    // 如果指定了id，就以指定的为准
                    if (!"".equals(beanName.trim())) {
                        ioc.put(beanName, aClass.newInstance());
                    } else {
                        // 如果没有指定，就以类名首字母小写
                        beanName = CommonUtils.lowerFirst(aClass.getSimpleName());
                        ioc.put(beanName, aClass.newInstance());
                    }

                    // service层往往是有接口的，面向接口开发，此时再以接口名为id，放入一份对象到ioc中，便于后期根据接口类型注入
                    Class<?>[] interfaces = aClass.getInterfaces();
                    for (Class<?> anInterface : interfaces) {
                        // 以接口的全限定类名作为id放入
                        ioc.put(anInterface.getName(), aClass.newInstance());
                    }
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    // 4、实现依赖注入
    private void doAutoWired() {

        // 有对象，再进行依赖注入处理
        if (ioc.isEmpty()) return;

        // 遍历 ioc 中所有对象，查看对象中的字段，是否有 @LyAutowired 注解，如果有需要维护依赖注入关系
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {

            // getFields()：获得某个类的所有的公共（public）的字段，包括父类中的字段。
            // getDeclaredFields()：获得某个类的所有声明的字段，即包括public、private和proteced，但是不包括父类的申明字段。

            // 获取bean对象中的字段信息
            Field[] declaredFields = entry.getValue().getClass().getDeclaredFields();

            // 遍历判断处理
            //  @LyAutowired  private IDemoService demoService;
            for (Field declaredField : declaredFields) {

                // 先判断有没有打上自动注入的注解
                if (!declaredField.isAnnotationPresent(LyAutowired.class)) {
                    continue;
                }

                // 有该注解
                LyAutowired annotation = declaredField.getAnnotation(LyAutowired.class);

                String beanName = annotation.value();  // 需要注入的bean的id

                if ("".equals(beanName.trim())) {
                    // 没有配置具体的bean id，那就需要根据当前字段类型注入（接口注入）  IDemoService
                    beanName = declaredField.getType().getName();
                }

                // 开启赋值
                declaredField.setAccessible(true);

                try {
                    declaredField.set(entry.getValue(), ioc.get(beanName));
                } catch (IllegalAccessException e) {
                    e.printStackTrace();
                }
            }
        }
    }

    // 5、构建url和HandlerMapping之间的关系
    private void initHandlerMapping() {
        if (ioc.isEmpty()) return;

        for (Map.Entry<String, Object> entry : ioc.entrySet()) {

            // 获取ioc中当前遍历的对象的class类型
            Class<?> aClass = entry.getValue().getClass();

            // 判断这个类到底有没有被Controller所标记
            if (!aClass.isAnnotationPresent(LyController.class)) {
                // 如果没有就下一个
                continue;
            }

            String baseUrl = "";

            // 再看看这个类有没有被RequestMapping标注
            if (aClass.isAnnotationPresent(LyRequestMapping.class)) {
                LyRequestMapping annotation = aClass.getAnnotation(LyRequestMapping.class);

                // 如果有可以要把它给记录下来的，相当于拿到了请求的路径
                baseUrl = annotation.value();
            }

            // 获取方法
            Method[] methods = aClass.getMethods();
            for (Method method : methods) {

                //  方法没有标识LyRequestMapping，就不处理
                if (!method.isAnnotationPresent(LyRequestMapping.class)) {
                    continue;
                }

                // 如果标识，就处理
                LyRequestMapping annotation = method.getAnnotation(LyRequestMapping.class);
                String methodUrl = annotation.value();
                String url = baseUrl + methodUrl;

                // 把method所有信息及url封装为一个Handler
                HandlerMapping handlerMapping = new HandlerMapping();
                handlerMapping.setMethod(method);
                handlerMapping.setPattern(Pattern.compile(url));
                handlerMapping.setController(entry.getValue());

                // 计算方法的参数位置信息
                Parameter[] parameters = method.getParameters();

                for (int j = 0; j < parameters.length; j++) {
                    Parameter parameter = parameters[j];
                    handlerMapping.getParamIndexMapping().put(parameter.getName(), j);
                }

                // 建立url和method之间的映射关系（map缓存起来）
                this.handlerMapping.add(handlerMapping);
            }
        }
    }

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        this.doPost(req, resp);
    }


    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

        // 获取的这个handler其实就相当于Controller
        HandlerMapping handler = this.getHandler(req);

        if (handler == null) {
            resp.getWriter().write("404 not found");
            return;
        }

        // 参数绑定
        // 获取所有参数类型数组，这个数组的长度就是我们最后要传入的args数组的长度
        Class<?>[] parameterTypes = handler.getMethod().getParameterTypes();

        // 根据上述数组长度创建一个新的数组（参数数组，是要传入反射调用的）
        Object[] paraValues = new Object[parameterTypes.length];

        // 以下就是为了向参数数组中塞值，而且还得保证参数的顺序和方法中形参顺序一致
        Map<String, String[]> parameterMap = req.getParameterMap();

        // 遍历request中所有参数（填充除了request，response之外的参数）
        for (Map.Entry<String, String[]> param : parameterMap.entrySet()) {

            String value = CommonUtils.join(param.getValue(), ",");  // 如同 1,2

            // 如果参数和方法中的参数匹配上了，填充数据
            if (!handler.getParamIndexMapping().containsKey(param.getKey())) {
                continue;
            }

            // 方法形参确实有该参数，找到它的索引位置，对应的把参数值放入paraValues
            Integer index = handler.getParamIndexMapping().get(param.getKey());//name在第 2 个位置

            paraValues[index] = value;  // 把前台传递过来的参数值填充到对应的位置去
        }

        // 最终调用handler的method属性
        try {
            handler.getMethod().invoke(handler.getController(), paraValues);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private HandlerMapping getHandler(HttpServletRequest req) {
        if (handlerMapping.isEmpty()) return null;

        // 当前请求的url
        String url = req.getRequestURI();
        System.out.println("当前访问的url：" + url);

        // 从HandlerMapping中看能否找到合适的能够处理这个请求的Handler
        for (HandlerMapping handler : handlerMapping) {
            Matcher matcher = handler.getPattern().matcher(url);
            if (!matcher.matches()) {
                continue;
            }
            return handler;
        }
        return null;
    }
}
